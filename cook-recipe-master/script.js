...
function addMeal(mealData, random = false) {
    ...
    meal.innerHTML = `
    <div class="meal-header" style="border:5px solid yellow"> 
...
async function showPopupArea(filter) {
    ...
    if (filter === 'area') {
        ...
        for (let i = 0; i < countryList.length; i++) {
            ...
            // const countryCode = getCountryCodeFromName(countryList[i]);
            countryEl.innerHTML = `
        <img src="${countryList[i]?.flags?.png}" alt="${countryList[i].name.common}">
        <span>${countryList[i]?.name?.common}</span>
        `;
            optionsEl.appendChild(countryEl);
            countryEl.addEventListener('click', async () => {
                areaFilterEl.innerHTML = `<img src="${countryList[i]?.flags?.png}" alt="${countryList[i].name.common}">
            <span>${countryList[i].name?.common}</span>`
...
if (filter === 'category') {
        ...
        const filteredCategoryList = categoryList.filter(cat => cat.strCategory.toLowerCase() !== 'beef');

        for (let i = 0; i < filteredCategoryList.length; i++) {
            const categoryEl = document.createElement('li');

            categoryEl.innerHTML = `
        <img src="./images/${filteredCategoryList[i].strCategory}.png" alt="${filteredCategoryList[i].strCategory}">
        <span>${filteredCategoryList[i].strCategory}</span>
        `;
            ...
            categoryEl.addEventListener('click', async () => {
                ...
                const mealsId = await getMealsIdByCategory(filteredCategoryList[i].strCategory);
...
const country = 'Indian';
const category = 'Seafood';

async function getFilteredMeals(country, category) {
  ...
  const countryMeals = (await countryRes.json()).meals || [];
  const categoryMeals = (await categoryRes.json()).meals || [];

  // Convert category meals to a Set of IDs for quick lookup
  const categoryMealIds = new Set(categoryMeals.map(meal => meal.idMeal));

  // Filter meals that are in both country and category
  const filteredMeals = countryMeals.filter(meal => categoryMealIds.has(meal.idMeal));

  console.log(filteredMeals);
  return filteredMeals;
}

const seleteddata = getFilteredMeals('Indian', 'Seafood');
